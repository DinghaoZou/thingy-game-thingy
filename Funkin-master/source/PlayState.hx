...
class PlayState extends MusicBeatState
{
	...
	// To Sonamaker: This variable is in the base game.
	private var vocals:FlxSound;
	private var vocalsFinished:Bool = false;

	// To Sonamaker: This variable is not in the base game. I added it.
	...
	var songTimeLength:Float;
	var timeElapsed:Float;

	var songTimeLengthMinutes:Float;
	var timeElapsedMinutes:Float;
	var songTimeLengthSeconds:Float;
	var timeElapsedSeconds:Float;
	...
	override public function create()
	{
		...
		timerTxt = new FlxText(healthBarBG.x, 10, healthBarBG.width, "", 30);
		timerTxt.setFormat(Paths.font("pixel.otf"), 30, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timerTxt.alignment = CENTER;
		if (PreferencesMenu.getPref('downscroll')) {timerTxt.y = 720 - timerTxt.fieldHeight - 10;}
	...
	override public function update(elapsed:Float)
	{
		...
		if (vocals.length == 0) { // vocals.length will be 0 if there's no Voices.ogg.
			timeElapsed = FlxMath.roundDecimal(inst.time / 1000, 0);
			songTimeLength = FlxMath.roundDecimal(inst.length / 1000, 0);
		} else {
			timeElapsed = FlxMath.roundDecimal(vocals.time / 1000, 0);
			songTimeLength = FlxMath.roundDecimal(vocals.length / 1000, 0);
		}

		timeElapsedMinutes = Math.ffloor(timeElapsed / 60);
		timeElapsedSeconds = timeElapsed % 60;
		// ---------------------------------------------------------------------------------------------
		songTimeLengthMinutes = Math.ffloor(songTimeLength / 60);
		songTimeLengthSeconds = songTimeLength % 60;

		timerTxt.visible = PreferencesMenu.getPref('timer');

		if (accuracyScoreDivider == 0)
		scoreTxt.text = "Score: " + songScore + " || Misses: " + songMisses + " || Accuracy: 0%";
		else
		scoreTxt.text = "Score: " + songScore + " || Misses: " + songMisses + " || Accuracy: " + (FlxMath.roundDecimal(((accuracyScore / accuracyScoreDivider) * 100), 2)) + "%";
		
		timerTxt.text = timeElapsedMinutes + ":" + StringTools.lpad(Std.string(timeElapsedSeconds), "0", 2) + " / " + songTimeLengthMinutes + ":" + StringTools.lpad(Std.string(songTimeLengthSeconds), "0", 2);
		//timerTxt.text = "Test";
